...
my_first_pat = '(([\w\.-]+)(\s.?\(f.*y.*)?( *(@|&.*;) *| +(at|AT|where|WHERE) +)([\w-]+|[\w-]+([\.;]| +[dD][oO]?[tT] +| +)[\w-]+)([\.;]| +(do?t|DO?[TM]) +| )(-?[eE]-?[dD]-?[uU]|com|COM)\W)|obfuscate\(\'([\w]+\.edu)\',\'([\w]+)\'\)'
phone_pattern = '\(?(\d{3})\)?[ -]?(\d{3})[ -](\d{4})\D+'
"""
TODO
...
def process_file(name, f):
    ...
    for line in f:
        ...
        for m in matches:
            ...
            #     print len(m)
            #     print name
            #     print '%s@%s.%s' % (m[1].replace("-", ""),
            #                         m[6].replace("cs ", "cs.").replace(
            #         "-", "").replace(" ", "").replace("dot", ".").replace(";", ".").replace("..", "."),
            #         m[10].replace("-", ""))
            if len(m[len(m) - 1]) != 0:
                email = "%s@%s" % (m[12], m[11])
            else:
